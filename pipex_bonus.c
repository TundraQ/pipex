...
/*   Updated: 2022/04/06 20:51:28 by azane            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	ft_here_doc(t_norm *norm, char **argv, int argc)
{
	int	result;
	...
	if (ft_strncmp(argv[1], "here_doc", ft_strlen("here_doc")) == 0)
	{
		...
		norm->file_out = open(argv[argc - 1], O_CREAT | O_WRONLY | O_APPEND,
				0777);
		while (1)
		{
...
{
	if (norm->i == 2 + norm->here_doc)
		dup2(norm->file_in, 0);
	if (norm->i > norm->here_doc + 2)
	{
...
void	ft_zero(t_norm *norm, int argc, char **argv, char *env[])
{
	norm->fifo[0][0] = -1;
	norm->fifo[0][1] = -1;
	norm->fifo[1][0] = -1;
...
int	main(int argc, char **argv, char *env[])
{
	...
	norm.i = 2;
	norm.here_doc = ft_here_doc(&norm, argv, argc);
	if (!norm.here_doc)
	{
		norm.file_out = open(argv[argc - 1], O_CREAT | O_WRONLY | O_TRUNC,
				0777);
		norm.file_in = open(argv[1], O_RDWR, 0777);
	}
	if (norm.file_in == -1 || norm.file_out == -1)
		ft_fatal("open");
	ft_zero(&norm, argc, argv, env);
	close(norm.fifo[1 - norm.cur_pipe][0]);
	norm.i = -1;
	while (++norm.i < argc - 3)
		wait(0);
